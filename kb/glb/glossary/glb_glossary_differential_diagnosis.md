---
id: GLB-GLOSSARY-021
schema_version: 2
title: Дифференциальная диагностика
domain: glb
category: glossary
body_system:
- multisystem
tags:
- glossary
- differential_diagnosis
urgency: routine
access_level: professional
target_specialist:
- all_specialties
age_group:
- all_ages
synonyms:
- дифференциальная диагностика
- дифдиагностика
- дифференциальный диагноз
relations:
- target: DIAG-DIFDIAG-001
  type: defines
  description: выведено из машиночитаемого слоя (refers_to)
- target: DIAG-DIFDIAG-002
  type: defines
  description: выведено из машиночитаемого слоя (refers_to)
- target: DIAG-DIFDIAG-003
  type: defines
  description: выведено из машиночитаемого слоя (refers_to)
term: Дифференциальная диагностика
definition: Процесс исключения заболеваний со сходной клинической картиной для установления наиболее вероятного диагноза.
refers_to:
- DIAG-DIFDIAG-001
- DIAG-DIFDIAG-002
- DIAG-DIFDIAG-003
sources:
- Международная классификация болезней 11-го пересмотра (МКБ-11). Всемирная организация здравоохранения, 2022.
- Клинические рекомендации «Стабильная ишемическая болезнь сердца». Минздрав РФ, 2024.
clinical_guidelines:
- КР «Стабильная ишемическая болезнь сердца», Минздрав РФ, 2024
last_medical_review: '2026-09-16'
medical_reviewer: модельная верификация (учебный проект)
author: Инженеры знаний №1 и №3
version: '2.1'
date_created: '2026-09-16'
date_updated: '2026-09-16'
status: approved
disclaimer: true
---

# Дифференциальная диагностика — Differential diagnosis

## Определение
Процесс исключения заболеваний со сходной клинической картиной для установления наиболее вероятного диагноза.

## Определение для пациента
Сравнение похожих болезней, чтобы понять, какая из них вероятнее.

## Синонимы и сокращения
дифференциальная диагностика, дифдиагностика, дифференциальный диагноз.

## Контекст использования
Термин используется в карточках: [[DIAG-DIFDIAG-001]], [[DIAG-DIFDIAG-002]], [[DIAG-DIFDIAG-003]]. Синонимы применяются поисковым модулем и распознаванием запросов на естественном языке.

## Источники
1. Международная классификация болезней 11-го пересмотра (МКБ-11). Всемирная организация здравоохранения, 2022.
2. Клинические рекомендации «Стабильная ишемическая болезнь сердца». Минздрав РФ, 2024.
