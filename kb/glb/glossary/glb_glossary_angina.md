---
id: GLB-GLOSSARY-028
schema_version: 2
title: Стенокардия
domain: glb
category: glossary
body_system:
- cardiovascular
tags:
- glossary
- angina
urgency: routine
access_level: professional
target_specialist:
- all_specialties
age_group:
- all_ages
synonyms:
- стенокардия
- грудная жаба
- ангинозная боль
relations:
- target: DIAG-SYMPTOM-002
  type: defines
  description: выведено из машиночитаемого слоя (refers_to)
- target: DIAG-DISEASE-002
  type: defines
  description: выведено из машиночитаемого слоя (refers_to)
term: Стенокардия
definition: Клинический синдром, проявляющийся давящей болью или дискомфортом за грудиной, провоцируемыми нагрузкой и купирующимися покоем или нитроглицерином.
refers_to:
- DIAG-SYMPTOM-002
- DIAG-DISEASE-002
sources:
- Международная классификация болезней 11-го пересмотра (МКБ-11). Всемирная организация здравоохранения, 2022.
- Клинические рекомендации Минздрава РФ, соответствующие нозологии (рубрикатор cr.minzdrav.gov.ru), редакции 2023–2024.
clinical_guidelines:
- Рубрикатор клинических рекомендаций Минздрава РФ
last_medical_review: '2026-09-16'
medical_reviewer: модельная верификация (учебный проект)
author: Инженеры знаний №1 и №3
version: '2.1'
date_created: '2026-09-16'
date_updated: '2026-09-16'
status: approved
disclaimer: true
---

# Стенокардия — Angina pectoris

## Определение
Клинический синдром, проявляющийся давящей болью или дискомфортом за грудиной, провоцируемыми нагрузкой и купирующимися покоем или нитроглицерином.

## Определение для пациента
Давящая боль за грудиной при нагрузке, которая проходит в покое.

## Синонимы и сокращения
стенокардия, грудная жаба, ангинозная боль.

## Контекст использования
Термин используется в карточках: [[DIAG-SYMPTOM-002]], [[DIAG-DISEASE-002]]. Синонимы применяются поисковым модулем и распознаванием запросов на естественном языке.
