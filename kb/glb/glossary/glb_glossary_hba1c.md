---
id: GLB-GLOSSARY-013
schema_version: 2
title: Гликированный гемоглобин (HbA1c)
domain: glb
category: glossary
body_system:
- endocrine
tags:
- glossary
- hba1c
urgency: routine
access_level: professional
target_specialist:
- all_specialties
age_group:
- all_ages
synonyms:
- HbA1c
- гликогемоглобин
- гликированный гемоглобин
relations:
- target: DIAG-EXAM-006
  type: defines
  description: выведено из машиночитаемого слоя (refers_to)
term: Гликированный гемоглобин (HbA1c)
definition: Доля гемоглобина, связанного с глюкозой; отражает средний уровень гликемии за 2–3 месяца.
refers_to:
- DIAG-EXAM-006
sources:
- Международная классификация болезней 11-го пересмотра (МКБ-11). Всемирная организация здравоохранения, 2022.
- Клинические рекомендации «Сахарный диабет 2 типа у взрослых». Минздрав РФ, 2022.
clinical_guidelines:
- КР «Сахарный диабет 2 типа у взрослых», Минздрав РФ, 2022
last_medical_review: '2026-09-16'
medical_reviewer: модельная верификация (учебный проект)
author: Инженеры знаний №1 и №3
version: '2.1'
date_created: '2026-09-16'
date_updated: '2026-09-16'
status: approved
disclaimer: true
---

# Гликированный гемоглобин (HbA1c) — Glycated hemoglobin

## Определение
Доля гемоглобина, связанного с глюкозой; отражает средний уровень гликемии за 2–3 месяца.

## Определение для пациента
Анализ крови, показывающий средний сахар крови за последние 3 месяца.

## Синонимы и сокращения
HbA1c, гликогемоглобин, гликированный гемоглобин.

## Контекст использования
Термин используется в карточках: [[DIAG-EXAM-006]]. Синонимы применяются поисковым модулем и распознаванием запросов на естественном языке.

## Источники
1. Международная классификация болезней 11-го пересмотра (МКБ-11). Всемирная организация здравоохранения, 2022.
2. Клинические рекомендации «Сахарный диабет 2 типа у взрослых». Минздрав РФ, 2022.
