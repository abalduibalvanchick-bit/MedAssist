---
id: GLB-DISCLAIMER-003
schema_version: 2
title: Предупреждение об экстренном состоянии
domain: glb
category: disclaimer
body_system:
- multisystem
tags:
- disclaimer
- emergency
- safety
urgency: emergency
access_level: professional
target_specialist:
- all_specialties
age_group:
- all_ages
relations: []
text: Выявлены признаки состояния, угрожающего жизни. Немедленно вызовите скорую медицинскую помощь (103 или 112). Не тратьте время на дальнейшие вопросы системе.
applies_to_access_level:
- professional
- nursing
- student
- patient
sources:
- Приказ Минздрава России от 20.06.2013 № 388н «Об утверждении Порядка оказания скорой медицинской помощи».
- European Resuscitation Council Guidelines 2021.
clinical_guidelines:
- ERC Guidelines, 2021
last_medical_review: '2026-09-16'
medical_reviewer: модельная верификация (учебный проект)
author: Инженеры знаний №1 и №3
version: '2.0'
date_created: '2026-09-16'
date_updated: '2026-09-16'
status: approved
disclaimer: true
---

# Предупреждение об экстренном состоянии

Выявлены признаки состояния, угрожающего жизни. Немедленно вызовите скорую медицинскую помощь (103 или 112). Не тратьте время на дальнейшие вопросы системе. Это предупреждение выводится решателем при срабатывании красного флага или распознавании неотложного состояния.
